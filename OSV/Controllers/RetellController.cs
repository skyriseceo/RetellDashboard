...
namespace OSV.Controllers
{
    ...
    public class RetellController : ControllerBase
    {
        ...
        [RetellSignatureAuth]
        public async Task<IActionResult> HandleRetellWebhook([FromBody] Requests.RetellWebhookPayload payload, CancellationToken cancellationToken)
        {
            if (payload?.Analysis == null || payload.Metadata == null || payload.Metadata.OurCustomerId == 0)
            {
                _logger.LogWarning("Webhook: Received invalid or incomplete payload.");
                return BadRequest(new ApiResponse(false, "Invalid payload. 'analysis' or 'metadata' missing."));
            }

            // (هنا هيدخل عشان يحدث الحالات زي Failed, NoAnswer)
            bool success = await _customerService.HandleWebhookCallbackAsync(payload, cancellationToken);
            if (!success)
            {
